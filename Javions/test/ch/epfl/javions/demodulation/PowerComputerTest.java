...
public class PowerComputerTest {
    ...
    @Test
    void returnsGoodArray()throws IOException{
        try(InputStream stream = new FileInputStream("resources/samples.bin")){
            PowerComputer i = new PowerComputer(stream,8);
            int[] a = new int[8];
    ...
    void returns2GoodArrays()throws IOException{
        try(InputStream stream = new FileInputStream("resources/samples.bin")) {
            PowerComputer i = new PowerComputer(stream, 8);
            int[] a = new int[8];
            int[] b = new int[8];

            int[] c = {73, 292, 65, 745, 98, 4226, 12244, 25722};
            int[] d = {36818, 23825};
            int j = i.readBatch(a);
            int k = i.readBatch(b);
            assertEquals(8, j);
            assertEquals(8,k);
            for (int l = 0; l < 8; l++) {
                assertEquals(c[l], a[l]);
            }
            for(int p = 0; p<2;p++){
                assertEquals(d[p],b[p]);
            }
        }
    }
}
